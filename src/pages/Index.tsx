...
const Index = () => {
  ...
  React.useEffect(() => {
    ...
    if (currentScreen === -1 && imageGenerationController.current) {
      ...
      // Only stop loading if unified system isn't handling it
  ...
  const unifiedAIStreaming = useUnifiedAIStreaming({
    ...
    onNewImage: async (imageUrl: string, prompt: string) => {
      ...
      // 🎯 NEW: Delay auto-generation by resetting counter when unified system generates image
      setLastAutoImageMessageCount(prev => {
        const currentUserCount = chatMessages.filter(msg => msg.type === 'user').length;
        // Reset to current message count, so auto-generation waits for full interval again
        const newCount = currentUserCount;
        const nextAutoTriggerAt = newCount + AUTO_IMAGE_TRIGGER_INTERVAL;
        
        console.log('📉 COMMUNICATION: Unified system generated image → delaying auto-generation:', {
          previousAutoCount: prev,
          currentUserMessageCount: currentUserCount,
          newAutoCount: newCount,
          nextAutoTriggerAt,
          messagesUntilNextAuto: nextAutoTriggerAt - currentUserCount,
          reason: 'unified_system_coordination',
          message: `Auto-gen will now trigger at message ${nextAutoTriggerAt} instead of continuing previous schedule`
        });
  ...
  const onGenerateImage = useCallback(async (prompt?: string) => {
    try {
      ...
      stopImageLoadingSound();
      playImageCompleteSound();
      // Stop loading animation only for automatic generation, not explicit requests, and not if unified system is handling it
        ...
        try {
          ...
          if (unifiedResponse && unifiedResponse.textContent) {
            ...
            if (unifiedResponse.hasImages && unifiedResponse.imageUrls.length > 0) {
              ...
              // 🛠️ CRITICAL FIX: Abort unified system to clear its loading state before legacy fallback
              unifiedAIStreaming.abortStream();
      ...
      if (isImageRequest) {
        try {
          ...
          setChatMessages(prev => {
            setLastMessageCount(prev.length + 1);
            playMessageSound();
            // Stop loading animation and reset explicit request flag - but not if unified system is handling it
            if (!unifiedAIStreaming.isGeneratingImage) {
              setIsGeneratingAdventureImage(false);
            }
            setIsExplicitImageRequest(false);
          ...
          // Stop loading animation and sound on error, reset explicit request flag - but not if unified system is handling it
          if (!unifiedAIStreaming.isGeneratingImage) {
            setIsGeneratingAdventureImage(false);
          }
          setIsExplicitImageRequest(false);
